  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-[#141414]/90 backdrop-blur-xl border-t border-white/5 px-6 pb-8 pt-2 flex justify-between items-end z-50 h-24 max-w-md mx-auto">
      ...
      >
        ...
        <span className={`text-[10px] font-bold ${getTextColor('form')} group-hover:text-white uppercase tracking-wider transition-colors`}>Form</span>
      </div>
      ...
      <div 
        ...
        className="flex flex-col items-center gap-1.5 flex-1 cursor-pointer group"
      >
        <FileText className={`${getIconColor('drills')} group-hover:text-white transition-colors`} size={24} fill={currentView === 'drills' ? "currentColor" : "none"} strokeWidth={currentView === 'drills' ? 0 : 2} />
        <span className={`text-[10px] font-bold ${getTextColor('drills')} group-hover:text-white uppercase tracking-wider transition-colors`}>Drills</span>
      </div>
